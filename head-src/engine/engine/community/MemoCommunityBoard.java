package engine.engine.community;
...
import engine.holders.AuctionItemHolder;
...
import net.l2jpx.gameserver.templates.L2EtcItem;
...
{
	private final static Logger LOGGER = Logger.getLogger(MemoCommunityBoard.class);
	...
	{
		System.out.println("Removendo item vendido: " + key + " do jogador: " + ph.getName());
	...
	{
		System.out.println("Removendo item de venda: " + id + " do jogador: " + ph.getName());
	...
	private String buyItemConfirm(PlayerHolder ph, PlayerHolder phOwner, int key)
	{
		if (!phOwner.getAuctionsSell().containsKey(key))
		{
			return "<br><br><br>Item ja nao esta disponivel.";
		}
		// Verifica se o jogador anunciante está online
		if (!L2World.getInstance().isPlayerOnline(phOwner.getObjectId()))
		{
			return "<br><br><br>O vendedor esta offline. A compra nao pode ser concluída.";
		}
	...
	private static void giveAuctionSold(L2PcInstance owner, AuctionItemHolder ih)
	{
		...
		if (!owner.isGM())
		{
			...
			owner.sendPacket(new CreatureSay(owner.getObjectId(), Say2.CRITICAL_ANNOUNCE, "Auction", "You have sold the item " + item.getName()));
			
			owner.getInventory().addItem("Auction", ih.getItemPriceId(), ih.getItemPriceCount(), owner, null);
		}
		else
		{
			LOGGER.warn("GM " + owner.getName() + " tentou duplicar o item ");
			
			owner.sendMessage("You cannot duplicate items.");
